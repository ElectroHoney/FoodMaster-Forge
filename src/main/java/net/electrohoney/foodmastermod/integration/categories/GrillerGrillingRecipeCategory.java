...
public class GrillerGrillingRecipeCategory implements IRecipeCategory<GrillerBlockRecipe> {
    public final static ResourceLocation UID = new ResourceLocation(FoodMaster.MOD_ID, "grilling");
    ...
    public GrillerGrillingRecipeCategory(IGuiHelper helper) {
        this.background = helper.createDrawable(TEXTURE, 0, 0, 176, 85);

        this.icon = helper.createDrawableIngredient(VanillaTypes.ITEM, new ItemStack(ModBlocks.GRILLER_BLOCK.get()));
    ...
    @Override
    public Class<? extends GrillerBlockRecipe> getRecipeClass() {
        return GrillerBlockRecipe.class;
    }

    @Override
    public Component getTitle() {
        return new TextComponent("Grilling");
    }
    ...
    @Override
    public void setRecipe(@Nonnull IRecipeLayoutBuilder builder, @Nonnull GrillerBlockRecipe recipe, @Nonnull IFocusGroup focusGroup) {

        int ingredientIndex = 0;
        for(int i = 0; i<=2; i++){
            for(int j = 0; j<=2; j++){
                if(ingredientIndex < recipe.getIngredients().size()){
                    builder.addSlot(RecipeIngredientRole.INPUT, 53-9+18*(i%3), 17+18*(j%3)).addIngredients(recipe.getIngredients().get(ingredientIndex));
                    ingredientIndex++;
                }
        ...
        builder.addSlot(RecipeIngredientRole.CATALYST, 134, 57).addIngredients(recipe.getUtensil());

        builder.addSlot(RecipeIngredientRole.OUTPUT, 134, 35).addItemStack(recipe.getResultItem());
    }

    @Override
    public void draw(GrillerBlockRecipe recipe, IRecipeSlotsView recipeSlotsView, PoseStack stack, double mouseX, double mouseY) {
    ...
    @Override
    public List<Component> getTooltipStrings(GrillerBlockRecipe recipe, IRecipeSlotsView recipeSlotsView, double mouseX, double mouseY) {
        int temperatureBarHeight = 55; //pixels, using I in comments is fun. I did stuff lol
        int maxPossibleTemperature = 300; //degrees? I just set an arbitrary amount identical to the block entity, also it has nothing to do with the maxTemperature from the recipe
        int tMax = recipe.maxTemperature * temperatureBarHeight / maxPossibleTemperature; //i stole this from my menu function
        int tMin = recipe.minTemperature * temperatureBarHeight / maxPossibleTemperature; //i stole this from my menu function
