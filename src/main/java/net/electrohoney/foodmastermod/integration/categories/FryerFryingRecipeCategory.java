...
public class FryerFryingRecipeCategory implements IRecipeCategory<FryerBlockRecipe> {
    public final static ResourceLocation UID = new ResourceLocation(FoodMaster.MOD_ID, "frying");
    ...
    public FryerFryingRecipeCategory(IGuiHelper helper) {
        this.background = helper.createDrawable(TEXTURE, 0, 0, 194, 85);

        this.icon = helper.createDrawableIngredient(VanillaTypes.ITEM, new ItemStack(ModBlocks.FRYER_BLOCK.get()));
    ...
    @Override
    public Class<? extends FryerBlockRecipe> getRecipeClass() {
        return FryerBlockRecipe.class;
    }

    @Override
    public Component getTitle() {
        return new TextComponent("Frying");
    }
    ...
    @Override
    public void setRecipe(@Nonnull IRecipeLayoutBuilder builder, @Nonnull FryerBlockRecipe recipe, @Nonnull IFocusGroup focusGroup) {

        int ingredientIndex = 0;
        for(int i = 0; i<=4; i++){
                if(ingredientIndex < recipe.getIngredients().size()){
                    builder.addSlot(RecipeIngredientRole.INPUT, 53+18*i, 53).addIngredients(recipe.getIngredients().get(ingredientIndex));
                    ingredientIndex++;
                }
        }
        ...
        builder.addSlot(RecipeIngredientRole.CATALYST, 89, 21).addIngredients(recipe.getUtensil());

        builder.addSlot(RecipeIngredientRole.OUTPUT, 160, 21).addItemStack(recipe.getResultItem());
    }

    @Override
    public void draw(FryerBlockRecipe recipe, IRecipeSlotsView recipeSlotsView, PoseStack stack, double mouseX, double mouseY) {
    ...
    @Override
    public List<Component> getTooltipStrings(FryerBlockRecipe recipe, IRecipeSlotsView recipeSlotsView, double mouseX, double mouseY) {
        int temperatureBarHeight = 55; //pixels, using I in comments is fun. I did stuff lol
        int maxPossibleTemperature = 200; //degrees? I just set an arbitrary amount identical to the block entity, also it has nothing to do with the maxTemperature from the recipe
        int tMax = recipe.maxTemperature * temperatureBarHeight / maxPossibleTemperature; //i stole this from my menu function
        int tMin = recipe.minTemperature * temperatureBarHeight / maxPossibleTemperature; //i stole this from my menu function
