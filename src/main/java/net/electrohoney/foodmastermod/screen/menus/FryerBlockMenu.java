...
import static net.electrohoney.foodmastermod.block.entity.custom.FryerBlockEntity.FRYER_DATA_SIZE;

public class FryerBlockMenu extends AbstractContainerMenu {
    ...
    public FryerBlockMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        //@todo the integer has to match the gecount from potblockentity constructor
        this(pContainerId, inv, inv.player.level.getBlockEntity(extraData.readBlockPos()),new SimpleContainerData(FRYER_DATA_SIZE));
    }

    public FryerBlockMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.FRYER_BLOCK_MENU.get(), pContainerId);
        checkContainerSize(inv, FryerBlockEntity.FRYER_ENTITY_CONTAINER_SIZE);
        blockEntity = ((FryerBlockEntity) entity);
        ...
        this.blockEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(handler -> {
            ...
            // Dish Slot
            this.addSlot(new SlotItemHandler(handler, 0, 89-9, 21));
            //Result Slot
            this.addSlot(new ModResultSlot(handler, 6, 160-9, 21));
        });
    ...
    private static final int TE_INVENTORY_SLOT_COUNT = FryerBlockEntity.FRYER_ENTITY_CONTAINER_SIZE;  // must be the number of slots you have!
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, ModBlocks.FRYER_BLOCK.get());
    }
