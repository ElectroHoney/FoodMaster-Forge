...
import static net.electrohoney.foodmastermod.block.entity.custom.InfuserBlockEntity.INFUSER_DATA_SIZE;
public class InfuserBlockMenu extends AbstractContainerMenu {
    ...
    public InfuserBlockMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        //@todo the integer has to match the gecount from potblockentity constructor
        this(pContainerId, inv, inv.player.level.getBlockEntity(extraData.readBlockPos()),new SimpleContainerData(INFUSER_DATA_SIZE));
    }

    public InfuserBlockMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.INFUSER_BLOCK_MENU.get(), pContainerId);
        checkContainerSize(inv, InfuserBlockEntity.INFUSER_ENTITY_CONTAINER_SIZE);
        blockEntity = ((InfuserBlockEntity) entity);
        this.level = inv.player.level;
        this.data = data;
        this.inputFluid = blockEntity.getInputFluid();
        this.outputFluid = blockEntity.getOutputFluid();
        ...
        this.blockEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(handler -> {
            ...
            this.addSlot(new ModResultSlot(handler, 1, 116, 49));
        });
    ...
    public int getScaledProgress(){
        ...
        int progressArrowSize = 16;//24 //Arrow height/length(in the video it was downwards mine is sideways) in pixels
    ...
    private static final int TE_INVENTORY_SLOT_COUNT = InfuserBlockEntity.INFUSER_ENTITY_CONTAINER_SIZE;  // must be the number of slots you have!
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, ModBlocks.INFUSER_BLOCK.get());
    }
